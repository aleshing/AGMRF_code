smoothDirect_extra <- function (data, Amat, X = NULL, formula = NULL, 
                                ...
                                hyper = c("pc", "gamma")[1], pc.u = 1, 
                                pc.alpha = 0.01, pc.u.phi = 0.5, 
                                pc.alpha.phi = 2/3, pc.u.cor = 0.7, 
                                pc.alpha.cor = 0.9, pc.st.u = NA, 
                                pc.st.alpha = NA, 
                                options = list(dic = TRUE, mlik = TRUE, 
                                               cpo = TRUE, 
                                               openmp.strategy = "default"), 
                                control.inla = list(strategy = "adaptive", 
                                                    int.strategy = "auto"), 
                                verbose = FALSE, geo = NULL, rw = NULL, 
                                ar = NULL) 
{
    if (!is.null(geo)) {
        message("Argument geo is deprecated in the smoothDirect function. 
                Only Amat is needed.")
    }
    is.temporal <- TRUE
    if (!is.null(rw) || !is.null(ar)) {
        ...
        message(paste0("Argument 'rw' and 'ar' have been deprecated in version 
                       1.0.0. They still work as intended for now. In the 
                       future, use time.model and st.time.model to specify 
                       temporal components\n e.g., time.model = '", time.model, 
        ...
        if (!is.temporal) {
            ...
            if (st.time.model == "rw1") {
                ...
                stop("Temporal interaction effect only support rw1, rw2, and 
                     ar1.")
            }
        }
    }
    ...
    if (is.yearly && is.main.ar) {
        stop("ar1 effects are not implemented with is.yearly = TRUE. Consider 
             using rw1 or rw2")
    }
    if (!is.null(Amat)) {
    ...
    if (!isTRUE(requireNamespace("INLA", quietly = TRUE))) {
        stop("You need to install the packages 'INLA'. Please run in your R 
    ...
    if (isTRUE(requireNamespace("INLA", quietly = TRUE))) {
        ...
        if (!is.spatial) {
            ...
            if (length(data) == 0) {
                stop("Spatial adjacency matrix is not provided and no 
                     observation labeled 'All' either.")
            }
        }
        else {
            data <- data[which(data$region != "All"), ]
        ...
        dat$region.struct <- dat$region.unstruct <- dat$region.int <- 
            dat$region_number
        if (is.yearly) {
            n <- year_range[2] - year_range[1] + 1
            if (n%%m != 0) 
                stop("The year_range specification is not a multiple of m. 
                     Please check that  year_range contains the first and last 
                     year of the periods used. For example, if the last period 
                     is 2015-2019, the second element in year_range should be 
                     2019.")
            nn <- n%/%m
            N <- n + nn
            rw.model <- INLA::inla.rgeneric.define(model = rw.new, n = n, m = m, 
                                                   order = rw, tau = exp(10), 
                                                   shape0 = a.rw, rate0 = b.rw)
            iid.model <- INLA::inla.rgeneric.define(model = iid.new, n = n, 
                                                    m = m, tau = exp(10), 
                                                    shape0 = a.iid, 
                                                    rate0 = b.iid)
            rw.model.pc <- INLA::inla.rgeneric.define(model = rw.new.pc, n = n, 
                                                      m = m, order = rw, 
                                                      tau = exp(10), u0 = pc.u, 
                                                      alpha0 = pc.alpha)
            iid.model.pc <- INLA::inla.rgeneric.define(model = iid.new.pc, 
                                                       ...
                                                       alpha0 = pc.alpha)
            if (is.spatial) {
                st.model <- INLA::inla.rgeneric.define(model = st.new, n = n, 
                                                       m = m, order = st.rw, 
                                                       S = region_count, 
                                                       Amat = Amat, 
                                                       type = type.st, 
            ...
            if (!is.null(Amat)) {
                inla.bym.constr.internal = 
                    utils::getFromNamespace("inla.bym.constr.internal", "INLA")
                R4 = Amat
                if (sum(R4 > 0 & R4 < 1) != 0) {
        ...
        if (is.yearly) {
            dat$time.unstruct <- dat$time.struct <- dat$time.int <- 
                years[match(dat$years, years[, 1]), 2]
        }
        else {
            dat$time.unstruct <- dat$time.struct <- dat$time.int <- 
                years[match(dat$years, years[, 1]), 2]
        ...
        time.area <- data.frame(region_number = x[, 2], time.unstruct = x[, 1], 
        ...
        if (survey.effect) {
            if ("survey" %in% colnames(data) == FALSE) 
                stop("survey.effect is set to TRUE, but no survey column in the 
                     input data")
            survey_count <- length(table(data$survey))
            if (survey_count <= 1) {
                warning("survey.effect is set to TRUE, but only one survey index 
                        is provided, the survey effect has been removed.")
                survey.effect <- FALSE
            }
        }
        newdata <- dat
        ...
        if (!is.null(X)) {
            ...
            if (length(covariate.names) == 0) {
                warning("The X argument is specified but no covariate 
                        identified.")
                X <- NULL
            }
                ...
                for (tt in unique(newdata$years)) {
                    for (ii in unique(newdata$region)) {
                        which <- intersect(which(X$years == tt), 
                                           which(X$region == ii))
                        if (length(which) == 0) {
                            stop(paste("Missing region-years in the covariate 
                                       matrix:", ii, tt))
                        }
                        if (length(which) > 1) 
        ...
        for (i in 1:N) {
            ...
            if ("mean.nohiv" %in% colnames(data)) {
                tmp$mean.nohiv <- tmp$lower.nohiv <- tmp$upper.nohiv <- 
                    tmp$var.est.nohiv <- tmp$logit.prec.nohiv <- 
                    tmp$logit.est.nohiv <- NA
            }
        ...
        if (is.null(formula)) {
            ...
            if (tolower(hyper) == "pc") {
                ...
                if (is.temporal) {
                    if (!is.yearly && !is.spatial) {
                        formula <- logit.est ~ 
                            f(time.struct, model = paste0("rw", rw), 
                              constr = TRUE, extraconstr = period.constr,
                              hyper = hyperpc1) + 
                            f(time.unstruct, model = "iid", hyper = hyperpc1)
                    }
                    else if (is.yearly && !is.spatial) {
                        formula <- logit.est ~ 
                            f(time.struct, model = rw.model.pc, 
                        ...
                        if (type.st == 1) {
                            ...
                            if (!is.ar) {
                                formula <- 
                                    update(formula, ~. 
                                           + f(region.int, model = "iid", 
                                               group = time.int,
                                               control.group = 
                                                   list(model = paste0("rw", 
                                                                       st.rw), 
                                                        scale.model = TRUE), 
                                               hyper = hyperpc1.interact))
                            }
                            ...
                            if (is.ar) {
                                ...
                                inla.scale.model.bym = 
                                    utils::getFromNamespace("inla.scale.model.bym", "INLA")
                                inla.bym.constr.internal = 
                                    utils::getFromNamespace("inla.bym.constr.internal", "INLA")
                                R2 <- inla.rw(N, order = st.rw, 
                                              scale.model = TRUE, sparse = TRUE)
                                R4 = Amat
                                if (sum(R4 > 0 & R4 < 1) != 0) {
                                    for (row in 1:nrow(R4)) {
                                        idx <- which(R4[row, ] > 0 & R4[row, 
                        ...
                        formula <- logit.est ~ 
                            ...
                            f(time.area, model = st.model.pc, diagonal = 1e-06, 
                              extraconstr = constr.st, values = 1:(N * S))
                    }
                }
                else {
                    formula <- logit.est ~ 
                ...
                if (is.temporal) {
                    if (!is.yearly && !is.spatial) {
                        formula <- logit.est ~ 
                            f(time.struct, model = paste0("rw", rw), 
                              param = c(a.rw, b.rw), constr = TRUE) + 
                            f(time.unstruct, model = "iid", 
                              param = c(a.iid, b.iid))
                    }
                    else if (is.yearly && !is.spatial) {
                        formula <- logit.est ~ 
                            f(time.struct, model = rw.model, diagonal = 1e-06, 
                              extraconstr = constr, values = 1:N) + 
                            f(time.unstruct, model = iid.model)
                    }
                    else if (!is.yearly && (is.spatial)) {
                        formula <- logit.est ~ 
                            f(time.struct, model = paste0("rw", rw), 
                              param = c(a.rw, b.rw), scale.model = TRUE,
                              extraconstr = period.constr) + 
                            f(time.unstruct, model = "iid", 
                              param = c(a.iid, b.iid)) + 
                        ...
                        if (type.st == 1) {
                            ...
                            formula <- 
                                update(formula, ~. 
                                       + f(time.area, model = "generic0", 
                                           Cmatrix = R, extraconstr = constr.st, 
                                           rankdef = N * S - (N - st.rw) * 
                                               (S - Q1$rankdef), 
                                           param = c(a.iid, b.iid)))
                        }
            ...
            if (!is.null(X)) {
                formula <- as.formula(paste("logit.est~", 
                                            as.character(formula)[3], "+", 
                                            paste(covariate.names, 
                                                  collapse = " + ")))
            }
        }
        if (is.main.ar) {
            formula <- 
                update(formula, ~. 
                       - f(time.struct, model = paste0("rw", rw), constr = TRUE, 
                           extraconstr = period.constr, hyper = hyperpc1) + 
                           f(time.struct, model = "ar", order = ar, 
                             hyper = hyperar1, extraconstr = period.constr, 
        ...
        if (is.spatial) {
            lincombs.info <- data.frame(Index = 1:(region_count * N), 
                                        District = NA, Year = NA)
            index <- 0
            for (j in 1:region_count) {
                for (i in 1:N) {
                    ...
                    space.time.id <- 
                        unique(time.area$time.area[time.area$time.unstruct ==
                    ...
                    if (!is.null(X)) {
                        if ("region" %in% by && (!"years" %in% by)) {
                            ...
                            which <- intersect(which(exdat$time.struct == i), 
                                               which(exdat$region.struct == j))
                        }
                    ...
                    lincombs.info[index, c("District", "Year")] <- c(j, 
                                                                     i)
                    if (!is.temporal) {
                        tmplin <- list(`(Intercept)` = 1, region.struct = area)
                        ...
                        assign(object.name, 
                               INLA::inla.make.lincomb(c(list(`(Intercept)` = 1, 
                                                              time.area = spacetime, 
                                                              time.struct = time, 
                                                              time.unstruct = time, 
                                                              region.struct = area), 
        ...
        fit <- INLA::inla(mod, family = "gaussian", control.compute = options, 
                          ...
                          control.family = 
                              list(hyper = list(prec = list(initial = log(1),
                                                            fixed = TRUE))), 
                          scale = exdat$logit.prec, lincomb = lincombs.fit, 
                          control.inla = control.inla, verbose = verbose)
        return(list(model = mod, fit = fit, Amat = Amat, newdata = exdat, 
                    time = seq(0, N - 1), area = seq(0, region_count - 1), 
                    time.area = time.area, survey.table = survey.table, 
                    ...
                    type.st = type.st, year_range = year_range, 
                    year_label = year_label, Amat = Amat, has.Amat = TRUE, 
                    is.temporal = is.temporal, lincombs.fit = lincombs.fit))
    }
}