...
print(xtable(summaries_sd_separate %>% select(-Model) %>% 
               filter(Parameter != "theta")), include.rownames = FALSE)

print(xtable(summaries_master %>%  
               filter(Model %in% c("Proposed", 
                                   "Smoothed Direct"))), 
      include.rownames = FALSE)
print(xtable(summaries_master %>%  
               filter(Model %in% c("Proposed Country-Intercept", 
...
# Smoothed direct and proposed model
...
ggsave(filename = "../Results/smoothed_direct_comp_no_country_int_1.pdf")
...
ggsave(filename = "../Results/smoothed_direct_comp_no_country_int_rb_1.pdf")
...
geo <- 
  rbind(
    cbind(geo_combined, 
          value = (out_master %>% 
                     filter(Model == 
                              "Smoothed Direct Country-Intercept"))$Estimate, 
          Model = "Smoothed Direct Country-Intercept"),
    cbind(geo_combined, 
          value = (out_master %>% 
                     filter(Model == "Proposed Country-Intercept"))$Estimate, 
          Model = "Proposed Country-Intercept")) %>%
  mutate(U5MR = 
           cut(value, c(-Inf, 0.04, 0.045, 0.05, 0.055, 0.06, 0.065, 0.07, 
                        0.075, 0.08, 0.085, 0.09, 0.095, 0.1, Inf),
...
plot_ests <- ggplot(geo) + geom_sf(aes(fill = U5MR)) +  
  scale_fill_viridis_d(option = 'E', direction = -1) + facet_wrap(~Model)  +
  geom_sf(data = geo_country, col = "black", fill = NA, lwd = 1)
ggsave(filename = "../Results/smoothed_direct_comp_country_int_1.pdf")
...
plot_ests_rb <- geo %>% filter(CNTRYNAMEE %in% c("Rwanda", "Burundi")) %>%
  ...
  geom_sf(data = geo_country %>% filter(country %in% c("Rwanda", "Burundi")),
          col = "black", fill = NA, lwd = 1)
ggsave(filename = "../Results/smoothed_direct_comp_country_int_rb_1.pdf")
...
ggsave(filename = "../Results/smoothed_direct_comp_no_country_int_2.pdf", 
       height = 10, width = 12)
...
out_master %>%
  ...
  filter(Model %in% c("Smoothed Direct Country-Specific", 
                      "Smoothed Direct",
                      "Proposed")) %>%
  filter(Country %in% c("Burundi", "Ethiopia", "Kenya")) %>%
  ggplot(aes(x = Estimate, y = region, col = Model)) + 
  geom_vline(aes(xintercept = country_direct), col = "red") +
  geom_point(aes(shape = borders_other_countries), 
             position = dodge, cex = 2) +
  ...
  labs(shape = "Borders Another Country") + 
  facet_wrap(facets = vars(Country), scales = "free") 
ggsave(filename = "../Results/smoothed_direct_comp_no_country_int_2a.pdf", 
       height = 7, width = 12)
...
out_master %>%
  ...
  filter(Model %in% c("Smoothed Direct Country-Specific", 
                      "Smoothed Direct",
                      "Proposed")) %>%
  filter(Country %in% c("Rwanda", "Tanzania", "Uganda")) %>%
  ggplot(aes(x = Estimate, y = region, col = Model)) + 
  geom_vline(aes(xintercept = country_direct), col = "red") +
  geom_point(aes(shape = borders_other_countries), 
             position = dodge, cex = 2) +
  ...
  labs(shape = "Borders Another Country") + 
  facet_wrap(facets = vars(Country), scales = "free") 
ggsave(filename = "../Results/smoothed_direct_comp_no_country_int_2b.pdf", 
       height = 7, width = 12)

out_master %>%
  mutate(Country = word(region, sep = "-")) %>%
  filter(Model %in% c("Smoothed Direct",
                      "Proposed")) %>%
  select(-c(lower, upper)) %>%
  pivot_wider(names_from = "Model",
              values_from = "Estimate") %>%
  ggplot(aes(x = `Smoothed Direct`, y = Proposed, col = Country)) + 
  geom_abline(slope = 1) + 
  geom_point() + 
  facet_wrap(facets = vars(borders_other_countries), ncol = 2)
...
out_master %>%
  ...
  labs(shape = "Borders Another Country") + 
  facet_wrap(facets = vars(Country), scales = "free") 
ggsave(filename = "../Results/smoothed_direct_comp_country_int_2.pdf", 
       height = 10, width = 12)
...
out_master %>%
  ...
  labs(shape = "Borders Another Country") + 
  facet_wrap(facets = vars(Country), scales = "free") 
ggsave(filename = "../Results/smoothed_direct_comp_country_int_2a.pdf", 
       height = 7, width = 12)
...
out_master %>%
  ...
  labs(shape = "Borders Another Country") + 
  facet_wrap(facets = vars(Country), scales = "free") 
ggsave(filename = "../Results/smoothed_direct_comp_country_int_2b.pdf", 
       height = 7, width = 12)

out_master %>%
  mutate(Country = word(region, sep = "-")) %>%
  filter(Model %in% c("Smoothed Direct Country-Intercept",
                      "Proposed Country-Intercept")) %>%
  select(-c(lower, upper)) %>%
  pivot_wider(names_from = "Model",
              values_from = "Estimate") %>%
  ggplot(aes(x = `Smoothed Direct Country-Intercept`, 
             y = `Proposed Country-Intercept`, col = Country)) + 
  geom_abline(slope = 1) + 
  geom_point() + 
  facet_wrap(facets = vars(borders_other_countries), ncol = 2)

#### Direct Estimates Maps ####

geo_direct <- cbind(geo_combined, 
                    value = direct_estimates_2010_2014$mean) %>%
  mutate(value_bin = 
           cut(value, c(-Inf, 0.04, 0.045, 0.05, 0.055, 0.06, 0.065, 0.07, 
                        0.075, 0.08, 0.085, 0.09, 0.095, 0.1, Inf),
               labels = c("<0.04", "0.04 to <0.045", "0.045 to <0.05",
                          ...
                          "0.08 to <0.85", "0.085 to <0.09", 
                          "0.09 to <0.95", "0.095 to <0.1", ">= 0.1")))
plot_direct <- ggplot(geo_direct) + 
  geom_sf(aes(fill = value_bin)) +  
  scale_fill_viridis_d(option = 'E', direction = -1) +
  labs(fill = "U5MR") +
  geom_sf(data = geo_country, col = "black", fill = NA, lwd = 1.5) +
  geom_sf_label(data = geo_country %>%
                  mutate(country = ifelse(country %in% c("Rwanda", "Burundi"),
                                          NA, country)), 
                aes(label = country),
                label.padding = unit(0.25, "lines"),
                size = 4)

plot_direct_rb <- geo_direct %>% 
  filter(CNTRYNAMEE %in% c("Rwanda", "Burundi")) %>%
  ggplot() + geom_sf(aes(fill = value_bin)) +  
  scale_fill_viridis_d(option = 'E', direction = -1) +
  labs(fill = "U5MR") +
  geom_sf(data = geo_country %>% filter(country %in% c("Rwanda", "Burundi")),
          col = "black", fill = NA, lwd = 1.5) +
  geom_sf_label(data = geo_country %>% 
                  filter(country %in% c("Rwanda", "Burundi")), 
                aes(label = country))

plot_direct_comb <- ggarrange(plot_direct, plot_direct_rb, nrow = 1, ncol = 2,
                              align = 'h', common.legend = TRUE)
ggsave(filename = "../Results/direct_map.pdf")
